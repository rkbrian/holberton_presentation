...
class MazeBuilder {
  ...
  constructor(width, height) {
    // maze defined by horizontal and vertical path measurements
    this.width = width;
    this.height = height;
    this.columns = this.width * 2 + 1;
    this.rows = this.height * 2 + 1;
    this.maze = this.initArray([]);

    const levels = {"Easy": 12, "Medium": 15, "Hard": 18, "Expert": 21}
  ...
  initArray(value) {
    return new Array(this.rows).fill().map(() => new Array(this.columns).fill(value));
  }
